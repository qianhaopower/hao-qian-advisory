#!/usr/bin/env python3
"""captions.json + fx.json -> CapCut draft — WORKING THEORY (English) variant.
Derived from scripts/xhs-pipeline/to_capcut.py (the FI/小红书 generator, 2026-09-25); same fx.json
schema and tracks, different text metrics: Latin fonts, and every size rule uses display WEIGHT
(Latin char = 0.55) instead of character count, so English lines render at the same visual size
as the 13字 Chinese lines the rules were tuned on. Keep the two files in step by hand.

Usage (inside the episode work dir, the pycapcut venv):
  "$HOME/Video Studio/work/venv-jy/bin/python" to_capcut_wt.py <source_ready.mp4> <draft_name> [drafts_dir]
"""
...
F_HEAVY = getattr(cc.FontType, "Poppins_Bold", None) or getattr(cc.FontType, "中黑体", None)   # captions, white toplines
F_BOLD = getattr(cc.FontType, "Rubik_Bold", None) or F_HEAVY   # gold punch / gold toplines
F_BRUSH = getattr(cc.FontType, "PermanentMarker_Regular", None) or F_HEAVY   # corner mark, floaters
F_TITLE = getattr(cc.FontType, "Poppins_Bold", None) or F_HEAVY
...
if ti:
    for i, (txt, col) in enumerate(ti["lines"]):
        seg = cc.TextSegment(
            ...
            style=TextStyle(size=(18.0 * min(1, 8 / max(1, _w(txt))) if col == "gold"
                                  else 16.0 * min(1, 9 / max(1, _w(txt)))), bold=True,
...
for c in CAPS:
    ...
    csize = min(8.5, max(5.2, 8.5 * 13 / max(_w(c["text"]), 1)))
    sc.add_segment(cc.TextSegment(
...
for tl in FX.get("toplines", []):
    ...
    seg = cc.TextSegment(
        ...
        style=TextStyle(size=10.5 * min(1, 14 / max(1, _w(tl["text"]))), bold=True,
